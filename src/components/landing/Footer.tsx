...
const Footer: React.FC = () => {
  return (
    <footer
      className="mt-12 md:mt-20 w-full bg-[#100713] border-t border-white/10 px-4 py-8 md:py-12 flex flex-col items-center text-white"
    >
      <div className="w-full max-w-[1080px] flex flex-col md:flex-row md:items-center md:justify-between gap-10 md:gap-0">
        ...
        <div className="flex flex-col items-center md:items-start gap-4 md:gap-3">
          <div className="flex items-center gap-2">
            <img src={LOGO_SRC} alt="Ping Logo" className="h-8 w-8 rounded-[6px] shadow-lg" />
            <span className="font-bold text-lg tracking-tight text-white">Ping</span>
          </div>
          <div className="flex gap-4 mt-2">
            {SOCIALS.map((social) => (
              <a
                ...
                className="hover:scale-110 transition-transform duration-150 text-white opacity-80 hover:opacity-100"
              >
                <social.icon size={22} />
              </a>
            ))}
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
          {footerLinks.map(link => (
            <a
              key={link.heading}
              href={link.href}
              className="flex items-center gap-2 text-white/90 hover:text-white font-medium transition-colors text-base"
            >
